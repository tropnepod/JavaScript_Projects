...
//initiate game, roll for turn, and determine active player
function startGame()    {
    var xTurn = 0;
    ...
    btnDisabled(btn); // disable start button since game has already started
    var btn = document.getElementById('btnStop');
    stopEnabled(btn); // enable stop button since game is started
...
function checkWinCon1(info, squareArray)    {
    ...
    if (match0Avatar != undefined && match1Avatar != undefined && match2Avatar != undefined)    {
        if (match0Avatar == match1Avatar && match0Avatar == match2Avatar)   {
            winDetected = "win"; // this flad will pass when a win has been detected
...
function square1Animate()   {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started, prevents avatar placement
        ...
        if (verdict == undefined)   { // if verdict is empty then the square is unoccupied
            var paintAvatar = determineAvatar(); // get the correct avatar for the active player
